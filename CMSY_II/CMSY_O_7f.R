...
## Note: In ID and Catch file, label "stock" was changed to "Stock"; in ID file, "comment" was changed to "Comment",
## In Catch file, label "TB" was changed to "bt"  
...
#-----------------------------------------
catch_file  <- "O_ICES&BS_Catch_7.csv" # name of file containing "stock", "yr", "ct", and optional "bt"
id_file     <- "O_ICES&BS_ID_8.csv"    # name of file containing stock-specific info and settings for the analysis
...
# If the line below is commented out (#), all stocks in the input file will be analyzed
 stocks <- c("sol-iris") #"sal-2231"    #c("sol-iris") # c("cod-scow") #c("cod-2224")   
...
if(write.output==T){
  outheaders = data.frame("Region", "Subregion","Name","SciName","Stock","start.yr","end.yr",
...
for(stock in stocks) {
  ...
  if(btype != "None" & length(bt[is.na(bt)==F])>=nab) {
    ...
    if(btype == "biomass" ) {
      ...
      jags.data        <- c('ct','bt','nyr', 'start.r','startbio','start.k',
                            'init.r','init.k', 'pen','b.yrs','b.prior')
  ...
  " # end main model
 , ifelse(user.log.r==F, 
   " #inverse relationship between the range of viable r and the height of the density function
     inverseRangeFactor <- 3/(start.r[2]-start.r[1])
     # give sigma some variability in the inverse relationship
     sigma.log.r         ~  dunif(0.00001*inverseRangeFactor,0.04*inverseRangeFactor)
     tau.log.r           <- pow(sigma.log.r,-2)
     log.rm              <- log((start.r[1]+start.r[2])/2) 
     r                   ~  dlnorm(log.rm,tau.log.r)   
    }", 
  " # second option for JAGS model deriving user r from range
  log.rm              <- mean(log(start.r))
  sigma.log.r         <- abs(log.rm - log(start.r[1]))/2
  tau.log.r           <- pow(sigma.log.r,-2)
  r                   ~  dlnorm(log.rm,tau.log.r)   
  } "),sep="") # end of ifelse() and paste() and of JAGS model for btype=="biomass"     
  ...
  # ---------------------------------------------------------------------
    ...
    jags.data        <- c('ct','bt','nyr', 'start.r', 'start.k', 'startbio', 'q.prior',
                          'init.q','init.r','init.k','pen','b.yrs','b.prior')
    # Parameters to be returned by JAGS
...
" # second option for JAGS model deriving user r from range
  ...
  tau.log.r           <- pow(sigma.log.r,-2)
  r                   ~  dlnorm(log.rm,tau.log.r)   
} "),sep="") # end of ifelse() and paste() and of JAGS model                  
...
if(mgraphs==T) {
  ...
  y.b_bmsy = all.b_bmsy[,nyr]} else {
   ...
   kernelF <- ci2d(x.F_Fmsy,y.b_bmsy,nbins=101,factor=2.1,ci.levels=c(0.50,0.80,0.75,0.90,0.95),show="none")
  c1 <- c(-1,100)
  c2 <- c(1,1)
  
  max.x1   <- max(c(2, max(F.Fmsy*1.05,max(kernelF$contours$"0.95"))))
  max.x    <- ifelse(max.x1 > 5,5,max.x1) 
